...
static QString SelectDirectory(QWidget *parent, QString title, QString path)
{
...
static QString SaveFile(QWidget *parent, QString title, QString path,
			QString extensions)
{
	QString file =
...
static QString OpenFile(QWidget *parent, QString title, QString path,
			QString extensions)
{
	QString file =
...
static QStringList OpenFiles(QWidget *parent, QString title, QString path,
			     QString extensions)
{
	QStringList files =
...
{
	setObjectName("VScrollArea");
	setStyleSheet("#VScrollArea {"
...
void OBSPropertiesView::RefreshProperties()
{
	...
	widget->setStyleSheet("#PropertiesContainer {"
			      "background-color: transparent;"
			      "}");
				 ...
				 QLabel **label)
{
	...
	if (stepVal < 1.0) {
		...
		const int decimals = ::std::min<int>(
			log10(1.0 / stepVal) + 0.99, sane_limit);
...
void WidgetInfo::ButtonClicked()
{
	...
	if (type == OBS_BUTTON_URL && strcmp(savedUrl, "") != 0) {
		...
		if (url.isValid() && (url.scheme().compare("http") == 0 ||
			...
			if (DisplayMessage(msg, true, true))
...
class EditableItemDialog : public QDialog {
	...
	void BrowseClicked()
	{
		...
		QString path = OpenFile(GetSettingsWindow(), QTStr("Browse"),
					curPath, filter);
		if (path.isEmpty())
			return;
...
void WidgetInfo::EditListAddFiles()
{
	...
	QStringList files = OpenFiles(GetSettingsWindow(), title,
...
void WidgetInfo::EditListAddDir()
{
	...
	QString dir = SelectDirectory(GetSettingsWindow(), title,
				      QT_UTF8(default_path));
#ifdef __APPLE__
	// TODO: Revisit when QTBUG-42661 is fixed
	widget->window()->raise();
...
void WidgetInfo::EditListEdit()
{
	...
	if (type == OBS_EDITABLE_LIST_TYPE_FILES) {
		...
		if (pathDir.exists())
			path = SelectDirectory(GetSettingsWindow(),
					       QTStr("Browse"), item->text());
		else
			path = OpenFile(GetSettingsWindow(), QTStr("Browse"),
