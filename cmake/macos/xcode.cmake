...
if(ENABLE_CCACHE AND CCACHE_PROGRAM)
  ...
  execute_process(
    COMMAND chmod a+rx "${CMAKE_CURRENT_BINARY_DIR}/ccache-launcher-c"
...
if(NOT CODESIGN_TEAM)
  ...
  if(CODESIGN_IDENTITY AND NOT CODESIGN_IDENTITY STREQUAL "-")
    # Switch to manual codesigning if a non-adhoc codesigning identity is
    # provided
    set(CMAKE_XCODE_ATTRIBUTE_CODE_SIGN_STYLE Manual)
    set(CMAKE_XCODE_ATTRIBUTE_CODE_SIGN_IDENTITY "${CODESIGN_IDENTITY}")
  else()
...
# Enable codesigning with secure timestamp when not in Debug configuration
...
# Enable codesigning with hardened runtime option when not in Debug
...
# Disable injection of Xcode's base entitlements used for debugging when not in
# Debug configuration (required for Notarization)
...
# Use DWARF with separate dSYM files when in Release or MinSizeRel
# configuration.
#
# * Currently overruled by CMake's Xcode generator, requires adding '-g' flag to
#   raw compiler command line for desired output configuration. Report to
#   KitWare.
#
...
# Make all symbols hidden by default (currently overriden by CMake's compiler
# flags)
...
#
# * CMake explicitly sets the version via compiler flag when transitive
#   dependencies require specific compiler feature set, resulting in the flag
#   being added twice. Report to KitWare as a feature request for Xcode
#   generator
...
set(CMAKE_XCODE_ATTRIBUTE_WARNING_CFLAGS
    "-Wvla -Wformat-security -Wno-inconsistent-missing-override -Wno-error=braced-scalar-init -Wno-error=unreachable-code-loop-increment"
)
